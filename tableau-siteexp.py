...
Usage:
  tableau-siteexp.py zsend [--noop] [-d]
  tableau-siteexp.py test [--noop] [-d]
  tableau-siteexp.py re [--noop] [-d]
  tableau-siteexp.py site <siteid> [--noop] [-d]
...
Options:
  ...
  site <siteid>    Run export site. tsm sites export
...
run_args_unlock = 'tsm sites unlock --id'
...
def main():
    ...
    if argz.get('zsend'):
        ...
        time.sleep(10)
        run_args = f"{run_args_unlock} {argz.get('<siteid>')}"
        run_args = pre_args + run_args
        l.info(f"Run \"{run_args}\"")
        if not argz.get('--noop'):
            exit_code = run_cmd(argz=run_args)
